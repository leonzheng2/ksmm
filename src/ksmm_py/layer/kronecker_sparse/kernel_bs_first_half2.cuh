...
void best_kernel_bs_first_half2(half *input, half *values, half *output, int batch_size, int a, int b, int c, int d, dim3 &blockGrid, dim3 &threadsPerBlock){
	while (1) {
        ...
        if (batch_size == 12544 && a == 1 && b == 512 && c == 3584 && d == 8) {
            ...
            blockGrid.x = 32;
            blockGrid.y = 196;
            kernel_bs_first_half2<128,32,64,8,16><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
...
//             blockGrid.y = 196;
        ...
        if (batch_size == 12544 && a == 896 && b == 32 && c == 16 && d == 1) {
            ...
            blockGrid.x = 896;
            blockGrid.y = 49;
            kernel_bs_first_half2<32,16,256,4,16><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
...
//             kernel_bs_first_half2<32,8,64,8,8><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
        ...
        if (batch_size == 12544 && a == 1 && b == 16 && c == 32 && d == 896) {
            ...
            blockGrid.x = 896;
            blockGrid.y = 49;
            kernel_bs_first_half2<16,16,256,4,8><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
//             threadsPerBlock.x = 32;
//             blockGrid.x = 896;
//             blockGrid.y = 196;
...
//             kernel_bs_first_half2<64,16,64,8,8><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
            ...
            blockGrid.x = 224;
            blockGrid.y = 196;
            kernel_bs_first_half2<128,32,64,8,8><<<blockGrid, threadsPerBlock>>>(input, values, batch_size, output, a, b, c, d);
            break;
        }
	}
}
