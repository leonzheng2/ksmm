...
from ksmm_py.benchmark.utils import get_dtype, set_device_and_get_device_name


def get_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument("--model-id", type=str, default="gpt2-xl-modified",
                        choices=['gpt2', 'gpt2-medium', 'gpt2-large', 'gpt2-xl', 'gpt2-xl-modified'])
    ...
    parser.add_argument("--precision", type=str, choices=["fp32", "fp16"], required=True)
...
def replace_linear_layers(model_id, module, up_pattern1, up_pattern2, down_pattern1, down_pattern2, algo, device):
    emb_dim_dict = {'gpt2': 768, 'gpt2-medium': 1024, 'gpt2-large': 1280, 'gpt2-xl': 1600, 'gpt2-xl-modified': 24 * 64}
    emb_dim = emb_dim_dict[model_id]
    for name, child in module.named_children():
        if isinstance(child, nn.Linear):
            if child.in_features == emb_dim and child.out_features == 4 * emb_dim and up_pattern1 is not None and up_pattern2 is not None:
                patterns = [up_pattern2, up_pattern1]
            elif child.in_features == 4 * emb_dim and child.out_features == emb_dim and down_pattern1 is not None and down_pattern2 is not None:
...
if __name__ == "__main__":
    ...
    if args.algo != "dense":
        replace_linear_layers(args.model_id, model, args.up_pattern1, args.up_pattern2, args.down_pattern1,
                              args.down_pattern2, args.algo, args.device)
    print(model)

    # Prepare input
    if args.model_id == "gpt2-xl-modified":
        model_id_for_tokenizer = "gpt2-xl"
    else:
        model_id_for_tokenizer = args.model_id
